...
function make_feed(options, callback) {
    ...
    var feed = new RSS({
        ...
        image_url:         options.imageUrl || url.join(options.url, 'coverart.jpg'),
        docs:              options.docs,
        ...
        custom_elements:   [
            ...
            {'itunes:image': {
                _attr: {
                    href: options.itunesImage || options.imageUrl || url.join(options.url, 'coverart.jpg')
                }
            }},
        ]
    });
    ...
    fs.readdir(path.join(options.path, 'items'), function(err, files) {
        ...
        for(var i = 0; i < files.length; i++) {
            ...
            feed.item({
                ...
                custom_elements: [
                    {'itunes:image': {
                        _attr: { href: url.join(options.url, 'thumbnails', videoId + '.jpg') }
                        }
                    },
                ]
            });
